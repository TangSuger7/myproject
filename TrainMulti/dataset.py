...
class MyData(data.Dataset):
    def __init__(self, datasets, data_size, is_train=True):
        ...
        dataset_root = config.data_root_dir #os.path.join(config.data_root_dir, config.task)
        ...
        self.label_paths = []
        for lie in lines:
            img, gt = lie.strip().split("   ")
            self.image_paths.append(img)
            self.label_paths.append(gt)
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            return image, label, class_label, [self.image_paths[index], self.label_paths[index]]
        else:
            return image, label, [self.image_paths[index], self.label_paths[index]]
