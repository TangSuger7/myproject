...
class Config():
    def __init__(self) -> None:
        ...
        self.data_root_dir = "/home/notebook/code/personal/80410839/sources/00group/dataset/Matting/Train/Stage3"
        ...
        self.testsets = {
            ...
            # Practical use
            'General': ','.join(['Stage1_Val']),
            'General-2K': '',
            'Matting': ','.join(['Stage1_Val']),
        }[self.task]
        ...
        self.training_set = {
            ...
            'General': '/home/notebook/code/personal/80410839/Matting/SAM2-UNeXT/train_sod.txt',
            'General-2K': '/home/notebook/code/personal/80410839/Matting/SAM2-UNeXT/train_stage1.txt',
            'Matting': '/home/notebook/data/group/00group/users/myjyf/HFHOME2/trainall.txt',
        }[self.task]
        ...
        self.num_outputs = 3  # Number of masks to output (1 for single mask, >1 for multi-mask)
        self.full_mask_lambda = 0.01  # Weight for full mask loss in multi-mask training
        self.decay_rate = 0.2  # Decay rate for full mask loss over epochs
        self.iou_pred_loss_weight = 0.05  # Weight for IoU prediction head loss in multi-mask training (参考NewImage)
        ...
        self.lr = (1e-5) * math.sqrt(self.batch_size / 4)     # DIS needs high lr to converge faster. Adapt the lr linearly
        ...
        # 30 24, 27, 29
        self.lr_decay_epochs = [24, 26, 28]    # Set to negative N to decay the lr in the last N-th epoch.
        ...
        if self.task in ['Matting']:
            self.lambdas_pix_last = {
                ...
                'cnt': 5 * 0,
                'structure': 20 * 1,
                'alpha_cons': 5 * 1,  # Alpha Consistency Loss: 减少脏预测，惩罚接近0.5的值
                'aleatoric': 1.0 * 0,  # Aleatoric Matting Loss: 不确定性感知的损失（已禁用，不再使用）
            }
            ...
            self.lambdas_pix_last = {
                ...
                'cnt': 5 * 0,
                'structure': 5 * 0,
                'alpha_cons': 10 * 1,  # Alpha Consistency Loss: 减少脏预测，惩罚接近0.5的值
                'aleatoric': 1.0 * 0,  # Aleatoric Matting Loss: 不确定性感知的损失（已禁用，不再使用）
            }
        else:
            self.lambdas_pix_last = {
