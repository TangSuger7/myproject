...
parser.add_argument('--labeled-id-path', type=str, default='/home/notebook/code/personal/80410839/Dataset/SPLIT2/labeled_mini.txt')
parser.add_argument('--unlabeled-id-path', type=str, default='/home/notebook/code/personal/80410839/Dataset/SPLIT2/unlabeled_mini.txt')
parser.add_argument('--save-path', type=str, default='exp/coco/unimatch_v2/dinov2_small/366')
...
def main():
    ...
    args.save_path = f"exp/{cfg['dataset']}/unimatch_v2/{cfg['backbone']}"
    # rank = 0
    # world_size = 1
    ...
    # state_dict = torch.load(f'./pretrained/cityscapes_unimatch_v2_1_16labels_dinov2_small.pth')
    ...
    from myloss import BCE_SSIM_LOSS
    from myloss2 import CrossEntropyWithL1
    criterion_l = BCE_SSIM_LOSS().cuda(local_rank)
    criterion_l2 = CrossEntropyWithL1(mode='multiclass').cuda(local_rank)
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            # import numpy as np
            # np.max(pred_x.cpu().detach().numpy())
            # np.max(mask_x.cpu().detach().numpy())
            import time
            # print(pred_x.shape)
            # print(mask_x.shape)
            a1 = time.time()
            ...
            a2 = time.time()
            loss_x = criterion_l2(pred_x, mask_x)
            a3 = time.time()
            print("ce+iou+ssim:",a2-a1)
            print("ce+iou+reg:",a3-a2)
            ...
            if (i % (max(1, len(trainloader_u) // 80)) == 0) and (rank == 0):
        ...
        mIoU, iou_class = evaluate(model, valloader, eval_mode, cfg, multiplier=cfg['crop_size'])
        mIoU_ema, iou_class_ema = evaluate(model_ema, valloader, eval_mode, cfg, multiplier=cfg['crop_size'])
        ...
        if rank == 0:
            ...
            torch.save(checkpoint, os.path.join(args.save_path, f"epoch{epoch}.pth"))
            torch.save(checkpoint, os.path.join(args.save_path, f"latest.pth"))
            if is_best:
