        ...
        for img, mask, id in loader:
            ...
            if mode == 'sliding_window':
                ...
                if multiplier is not None:
                    ...
                    if multiplier == cfg['crop_size']:
                        new_h, new_w = cfg['crop_size'], cfg['crop_size']
                    else:
            ...
            imgt_ = Image.open(os.path.join(cfg['data_root'],id[0].split()[0]))
            imgt_ = imgt_.resize([ori_w, ori_h])
            img2 = np.array(imgt_)
            # pred_ = transforms.Resize(imgt_.size[::-1])(pred)

            m1 = pred.permute(1,2,0).contiguous().repeat(1,1,3).contiguous().cpu()

            alpha = 0.5
            color = np.array([0.49,0.04,0.41]).reshape((1,1,3)) * 255
            color = color.repeat(img2.shape[0], 0)
            color = color.repeat(img2.shape[1], 1)
            gt = np.where(m1 > 0.5, img2*(1-alpha)+color*alpha, img2)

            img_res = np.concatenate((img2,m1*255, gt),axis=1).astype('uint8')
            # 先查看epoch
            idx = 0
            while 1:
                if os.path.exists(os.path.join("log", str(idx),  os.path.basename(id[0].split()[0]))):
                    idx += 1
                else:
                    Path(os.path.join("log", str(idx))).mkdir(parents=True, exist_ok=True)
                    Image.fromarray(img_res).save(os.path.join("log", str(idx),  os.path.basename(id[0].split()[0])))
                    break
